...
\begin{document}
    ...
    \begin{figure}[H]
        ...
        \includegraphics[scale=0.5]{polimi.png} \hfill \includegraphics[scale=0.5]{heap.png}
    \end{figure}
    ...
    \begin{multicols}{2}
        ...
        \begin{itemize}
            ...
            \item \textbf{Compilation}: Each model was compiled twice using \texttt{iree-compile} to isolate the performance impact of vectorization.
        \end{itemize}
        ...
        % --- SINGLE COLUMN TABLE (Fixes Layout Issues) ---
        \begin{table}[H]
            \centering
            \caption{Scalar vs. Vector Benchmarks.}
            \vspace{2mm}
            % Fits table to column width
            \resizebox{\linewidth}{!}{
                \begin{tabular}{llrrr} 
                    \toprule
                    \textbf{Model} & \textbf{Type} & \textbf{Time} & \textbf{Mem} & \textbf{Instr} \\
                     & & (ms) & (KB) & (Vec) \\
                    \midrule
                    \multirow{2}{*}{MobileNet} & Scalar & 10,516 & 208,248 & 0 \\
        ...
        \begin{description}[leftmargin=0pt,labelindent=0pt,style=nextline]
          ...
          AlexNet, CaffeNet, GoogleNet~\cite{googlenet}, and \textbf{Network in Network (NiN)} showed \textbf{0 vector instructions}. The failure of NiN is particularly notable. Despite introducing $1\times1$ ``Mlpconv'' layers, the architecture's large $11\times11$ input kernel with stride 4 produces strided memory access patterns that inhibit LLVM vectorization.
        
        \end{description}

        \section{Discussion}
        
        \subsection{Emulation Overhead \& Docker Analysis}
        ...
        \begin{figure}[H]
            \centering
            % Matches uploaded filename
            \includegraphics[width=\linewidth]{docker-usage.png}
            \caption{Host Resource Usage. Blue spikes indicate high CPU load during QEMU vector emulation.}
            \label{fig:docker}
        \end{figure}
        ...
        \textbf{1. High-End Edge (e.g., Jetson Orin, RPi 5):} 
        ...
        \textbf{2. Mid-Range Edge (e.g., RPi Zero 2 W):} 
        ...
        \textbf{3. Low-Power/Microcontrollers (e.g., STM32):} 
        ...
        To contextualize these results, Table~\ref{tab:sim_vs_silicon} highlights the fundamental differences between our experimental simulation and theoretical silicon performance. Future work should involve deployment on physical hardware (e.g., SiFive X280) to validate the expected 4x-8x performance gains masked by simulation overhead.
